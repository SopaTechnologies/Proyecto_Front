...
export class GameBoardComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.gameStateSubscription = this.gameApi.getGameStateObservable().subscribe(state => {
      if (state) {
    ...
    this.gameApi.getGameState(this.gameId).subscribe(state => {
      ...
      if (this.isMyTurn()) {
        const myUnits = this.gameState.units.filter(u => u.owner === this.playerIndex);
        if (myUnits.length > 0) {
  ...
  renderGame(): void {
    ...
    this.tiles.forEach(tile => {
      ...
      const isSelected = !!(this.selectedUnit &&
      ...
      this.drawHex(ctx, x, y, this.hexSize, tile.color, isSelected, isReachable, isAttackable);
    });
  ...
  private drawUnit(ctx: CanvasRenderingContext2D, x: number, y: number, unit: GameUnit): void {
    ctx.save();
    ctx.translate(x, y);
  ...
  onCanvasClick(event: MouseEvent): void {
    ...
    const clickedUnit = this.gameState.units.find(u => u.q === q && u.r === r);
  ...
  attackUnit(attackerId: string, defenderId: string): void {
    ...
    this.gameApi.attack(this.gameId, attackerId, defenderId).subscribe({
      next: (result) => {
        const msg = `${result.attacker.name} attacked ${result.defender.name}! ` +
